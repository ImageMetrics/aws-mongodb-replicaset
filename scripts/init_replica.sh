...
if [ "$version" == "4.0" ] || [ "$version" == "4.2" ] || [ "$version" == "5.0" ] || [ "$version" == "6.0" ];
then
...
if [[ "$NODE_TYPE" == "Primary" ]]; then
    ...
    do
        ...
        while [ true ]; do
            ...
            echo "mongosh --quiet --host ${addr} --port ${port}"

mongosh --quiet --host ${addr} --port ${port} << EOF
use admin
EOF
...
EOF
    ...
    # wait for mongosh --quiet to become primary
    sleep 10
    check_primary true
